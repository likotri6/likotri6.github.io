(function() {
	// STEP 1 : GET the variables of htmlFile(h), Bucket(b), gID(g), page reloaded(rl) --from the url
	...
	// STEP 2 : Get url path variables
	(/(\d+)\.html$/ ['exec'](location.pathname)) ? /(\d+)\.html$/ ['exec'](location.pathname)[1] : '';
	
	
	// STEP 3 : Get host cookies using 1 liner script
	...
	if (cookieObj.get('pg') == 'daily')
	{
		...
		// STEP 5 : WAIT FOR DATA from above
		async function asyncWait() 
		{
			const result = await getData();
			
			// STEP 6 : SET COOKIE function
			...
			setTimeout(function ()  //Wait for variables to load
			{
				...
				{	
					...
					{
						//report 3rd visit to stats.php
						var http = new XMLHttpRequest();
						var url = server + '/resources/js/stats.js';
						var params = 'gV3=1&h=' + _0xbe6fx1.h + '&b=' + _0xbe6fx1.b;
